...
def main():
    ...
    if mood == 'warm':
        main_bgm = random.choice(WARM_BGM)
        sub_bgm  = random.choice(TENSION_BGM)
    else:
        main_bgm = random.choice(TENSION_BGM)
        sub_bgm  = random.choice(WARM_BGM)

    print(f"Mood: {mood}")
    print(f"Main BGM: {main_bgm['file']}")
    print(f"Sub BGM:  {sub_bgm['file']}")
    ...
    # 下載 BGM
    main_url = f"{GITHUB_RAW}/{requests.utils.quote(main_bgm['file'])}"
    sub_url  = f"{GITHUB_RAW}/{requests.utils.quote(sub_bgm['file'])}"
    download_file(main_url, "main.mp3")
    download_file(sub_url,  "sub.mp3")
    ...
    # Step 2: 裁切主軌 BGM
    subprocess.run(["ffmpeg", "-y", "-ss", main_bgm["start"], "-t", main_bgm["duration"],
                    "-i", "main.mp3", "-c:a", "aac", "-b:a", "128k", "main_trim.aac"], check=True)

    # Step 3: 裁切副軌 BGM
    subprocess.run(["ffmpeg", "-y", "-ss", sub_bgm["start"], "-t", sub_bgm["duration"],
                    "-i", "sub.mp3", "-c:a", "aac", "-b:a", "128k", "sub_trim.aac"], check=True)
